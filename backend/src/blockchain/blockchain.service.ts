...
export class BlockchainService implements OnModuleInit {
    ...
    onModuleInit() {
        try {
            const rpcUrl = this.configService.get<string>('BLOCKCHAIN_RPC_URL') || this.configService.get<string>('SEPOLIA_RPC_URL') || 'http://127.0.0.1:8545';
            const contractAddress = this.configService.get<string>('CONTRACT_ADDRESS');
            const privateKey = this.configService.get<string>('CONTRACT_PRIVATE_KEY') || this.configService.get<string>('PRIVATE_KEY');
